...
def solution(node, idx):
    ...
    # ヽ(´▽`)/
    # Если удаляем головной элемент (индекс 0)
    if idx == 0:
        return node.next_item  # Возвращаем следующий элемент как новую голову

    # Инициализируем текущий узел и счетчик
    current = node
    previous = None
    current_index = 0
    ...
    # Если нашли элемент с нужным индексом
    if current is not None:
        # Обходим элемент, связывая предыдущий с следующим
