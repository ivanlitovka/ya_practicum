...
def load_data_to_stage(s3_paths):
    for key, value in s3_paths.items():
        if key == 'customer_research':
            ...
            columns = ['date_id', 'category_id', 'geo_id', 'sales_qty', 'sales_amt']
            ...
            columns = ['date_time', 'city_id', 'city_name', 'customer_id', 'first_name', 'last_name', 'item_id', 'item_name', 'quantity', 'payment_amount']
        ...
        for i in range(0, len(load_csv), step):
            ...
            try:
                ...
                conn.commit()
                print(f"Загружено {len(data_tuples)} строк (порция {i // step + 1})")
