...
def extract_load_users():
    ...
    try:
        src_engine = create_engine(source_uri)
        df = pd.read_sql('SELECT * FROM users', con = src_engine)
    except Exception as e:
        log.error(f'Ошибка при чтении источника: {e}', exc_info = True)
        raise
    ...
    with dwh_engine.begin() as conn:
        #Clear stg (full rewrite)
        log.info('Выполняем TRUNCATE TABLE stg.bonussystem_users RESTART IDENTITY...')
        conn.execute(text('TRUNCATE TABLE stg.bonussystem_users RESTART IDENTITY'))

        #Insert data
        log.info(f'Начинаем вставку {rows_count} строк в stg.bonussystem_users.')
        try:
            df.to_sql(
                name = 'bonussystem_users',
                schema = 'stg',
                con = conn,
                if_exists = 'append',
                index = False,
            ...
            log.info('Данные успешно загружены в stg.bonussystem_users.')
        except Exception as e:
            log.error(f'Ошибка при вставке данных в DWH: {e}', exc_info = True)
            raise
